# Level 6 — SecurityAudit 읽기 권한 남용

> **URL**: http://level6-cc4c404a8a8b876167f5e70a7d8c9880.flaws.cloud/
...
사이트가 **직접 자격증명을 준다** — "어디까지 할 수 있는지 한 번 봐보라" 는 뉘앙스로. 이 키에는 **`SecurityAudit`** AWS 관리형 정책이 붙어 있다. 이 정책만으로도 숨어 있는 Lambda 함수를 발견하고, 비밀 API Gateway URL 을 알아내 플래그 페이지에 도달한다.

## 🧭 공식 힌트

사이트가 바로 아래 자격증명을 제공한다(공식 공개 값):
```
AKIAJFxx...redacted...Q3OBGA
S2Ipymxx...redacted...TVApCZ
```
...
> SecurityAudit 은 읽기 권한입니다. 그런데 "읽기" 만으로도 Lambda, API Gateway, 누가 누구에게 호출 권한을 주었는지 다 들여다볼 수 있습니다.
...
- IAM 관리형 정책 `SecurityAudit` 은 리소스의 **메타데이터**를 전부 읽게 해 준다 (list·get·describe 류). 하지만 "데이터" 자체는 못 본다.
...
"SecurityAudit 은 단지 읽기 아닌가요?" — 맞다. 하지만:
- `lambda:GetFunction*`, `lambda:ListFunctions`, `lambda:GetPolicy` 는 SecurityAudit 에 포함된다.
...
```bash
aws lambda get-policy --function-name Level6 --profile level6 --region us-west-2 | jq -r '.Policy | fromjson'
```
```json
{
  "Version": "2012-10-17",
...
방문하면 축하 메시지와 함께 **flaws.cloud 완주 페이지** 가 나온다.

```bash
curl -s https://s33ppypa75.execute-api.us-west-2.amazonaws.com/Prod/level6 | head -20
```
...
- 최종 URL: **https://s33ppypa75.execute-api.us-west-2.amazonaws.com/Prod/level6/**
- 교훈:
